...
",
    ...
    FROM (
        ...
        FROM 
            ...
            INNER JOIN division dv ON d.division_id = dv.division_id
            WHERE
                s.department_id = '{$filterData['departmentId']}' AND
                {$baseConditions}
            GROUP BY 
                s.name, 
                DATEPART(ISO_WEEK, ot_record.date)
    ...
    FROM (
        ...
        FROM 
            ...
            INNER JOIN division dv ON d.division_id = dv.division_id
            WHERE
                d.division = '{$filterData['divisionId']}' AND
                {$baseConditions}
            GROUP BY 
                d.name, 
                DATEPART(ISO_WEEK, ot_record.date)
...
if ($filterData) {
    ...
    if (!empty($filterData['sectionId'])) {
        $selectedSubquery = $subqueries['section'];
        // Modify the subquery for section condition here
    } elseif (!empty($filterData['departmentId'])) {
        $selectedSubquery = $subqueries['department'];
        // Modify the subquery for department condition here
    } elseif (!empty($filterData['divisionId'])) {
        $selectedSubquery = $subqueries['division'];
        // Modify the subquery for division condition here
    } else {
        $selectedSubquery = $subqueries['default'];
    }
} else {
    $selectedSubquery = $subqueries['default'];
}
